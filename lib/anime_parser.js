...
const BASE_URL2 = 'https://gogoanime.hu/';
const ajax_url = 'https://ajax.gogo-load.com/';
const anime_info_url = 'https://gogoanime.hu/category/';
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    const animePageTest = await axios.get(`https://gogoanime.hu/category/${id}`);
...
export const scrapeWatchAnime = async ({ id }) => {
  try {
    ...
    const WatchAnime = await axios.get(`https://gogoanime.hu/${id}`);
