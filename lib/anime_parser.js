...
const BASE_URL = 'https://gogoanimes.cv/';
const BASE_URL2 = 'https://gogoanimes.cv/';
const ajax_url = 'https://ajax.gogo-load.com/';
const anime_info_url = 'https://gogoanimes.cv/category/';
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    const animePageTest = await axios.get(`https://gogoanimes.cv/category/${id}`);
...
export const scrapeWatchAnime = async ({ id }) => {
  try {
    ...
    const WatchAnime = await axios.get(`https://gogoanimes.cv/${id}`);
